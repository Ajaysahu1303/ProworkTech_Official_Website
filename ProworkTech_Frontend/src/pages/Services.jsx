...
const Services = () => {
  ...
  useEffect(() => {
    const timer = setInterval(() => {
      setIndex((prevIndex) => (prevIndex + 1) % serviceTitles.length);
    }, 3000);
          ...
          <div className="space-y-24 md:space-y-32">
            {detailedServices.map((service, index) => (
              <motion.div
                ...
                className={`grid lg:grid-cols-2 gap-12 lg:gap-20 items-center ${index % 2 !== 0 ? 'lg:flex-row-reverse' : ''
                  }`}
              >
                <div className={index % 2 !== 0 ? 'lg:order-2' : ''}>
                  <div className={`w-16 h-16 rounded-2xl ${service.lightColor} ${service.textColor} flex items-center justify-center mb-8 border border-primary-100/50 shadow-sm`}>
                    {service.icon}
                  </div>
                  <h2 className="heading-md mb-4 text-[var(--color-text-primary)]">{service.title}</h2>
                  <p className={`text-lg font-bold ${service.textColor} mb-6`}>{service.tagline}</p>
                  ...
                  <div className="grid sm:grid-cols-2 gap-4">
                    {service.features.map((feature, fIdx) => (
                      <div key={fIdx} className="flex items-center gap-3 text-primary-700 font-medium">
                        <CheckCircle2 className={`${service.textColor} flex-shrink-0`} size={20} />
                        {feature}
                      </div>
                    ))}
                  </div>
                  <div className="mt-8">
                    <Link 
                      ...
                      className={`inline-flex items-center gap-2 px-8 py-3 rounded-xl shadow-lg font-bold text-white transition-all duration-300 transform hover:-translate-y-1 ${service.color} hover:shadow-xl hover:opacity-90`}
                    >
                      Book Service
                    </Link>
                  </div>
                ...
                <div className={`relative ${index % 2 !== 0 ? 'lg:order-1' : ''}`}>
                  <div className={`absolute -inset-10 ${service.lightColor} rounded-full -z-10 blur-3xl opacity-40`}></div>
                  <motion.div
                    whileHover={{ y: -10 }}
